...
def revoke_google_access():
    ...
    revoke = requests.post('https://accounts.google.com/o/oauth2/revoke',
        params={'token': credentials.token},  # noqa
...
def index():
    ...
    latest_items = session.query(CatalogItem).order_by(desc(CatalogItem.id))[0:10]  # noqa
    user = get_user()
...
def login():
    ...
    # indicate the api server will redirect the user
    # after the user completes authorization
    flow.redirect_uri = url_for('gconnect', _external=True)
    authorization_url, state = flow.authorization_url(
        # Enable offline access so that our app
        # can refresh an access token without
...
def show_item(category_id, item_id):
    catalog_item = session.query(CatalogItem).filter_by(category_id=category_id, id=item_id).one_or_none()  # noqa
    user = get_user()
...
def add_item():
    ...
    if request.method == "POST":
        if request.form['name'] and request.form['description']:
            ...
            item.category_id = get_category_by_name(request.form['category']).id  # noqa
            ...
            return render_template("add.html",
                                   ...
                                   prev_selected_category=request.form['category'],  # noqa
                                   categories=categories)
...
def api_list_category(category_name):
    result = {}
    category = session.query(Category).filter_by(name=category_name.capitalize()).one_or_none()  # noqa
    if category:
        items = session.query(CatalogItem).filter_by(category_id=category.id)
        result[category.name] = []
        for i in items:
